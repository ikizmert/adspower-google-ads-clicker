...
async function setupImageBlocking(page) {
  ...
  page.on("request", (req) => {
    ...
    const url = req.url();
    // Google sayfalarında image yüklensin (fingerprint için gerekli, bot sinyali olmasin).
    // Reklam landing page'lerinde (adelcicek.com vb.) image blokla (sadece browseAdPage scroll/mouse).
    ...
    // Landing page'de image/media/font blok
...
async function humanTypeAndSubmit(page, query, tag = "") {
  // Simplified — hb-test'in working approach'unu birebir taklit eder.
  // Önceki "humanization" (triple-click, backspace, dropdown click) bot imzası bırakiyordu.
  const searchInput = await page.$('textarea[name="q"], input[name="q"]');
  if (!searchInput) return false;

  await searchInput.click().catch(() => {});
  await randomSleep(0.3, 0.7);
  await searchInput.type(query, { delay: 80 + Math.random() * 120 });
  await randomSleep(0.8, 1.4);

  // Direkt Enter — dropdown click yok (hb-test'te de yoktu, çalışıyordu)
  try {
    await Promise.all([
